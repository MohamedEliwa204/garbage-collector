...
int main(){
    FILE* input_csv = fopen("testcases.csv", "r");
    ...
    //header
    fprintf(output_csv, "testcase_id,reference_counting-json,mark_sweep_json,mark_compact.json\n");
    ...
    while (fgets(line, sizeof(line), input_csv))
    {
        ...
        *comma_ptr = '\0';
        ...
        if (build_environment(root_json, &env))
        {
            ...
            fprintf(output_csv, "%s,%s,%s,%s", testcase_id, rc_csv, ms_csv, mc_csv);
            free(rc_result);
            free(ms_result);
            free(mc_result);
        }
...
HeapObject* find_object_by_name(GCEnvironment* env, const char* name){
    for (size_t i = 0; i < env->object_count; i++)
    {
